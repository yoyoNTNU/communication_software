...
RSpec.describe "Auth::Sessions", type: :request do
  ...
  describe "POST /auth/member/sign_in" do
    example "failed to sign in with uncertified email" do
      m=Member.new(user_id:"test1",name:"test1",phone:"09123456789",email:"example1@email.com",password:"Example124", is_login_mail: false)
      m.save
      post "/auth/member/sign_in",
      params:{email: "example1@email.com", password:"Example124"}
      expect(response).to have_http_status(401)
      expect(JSON.parse(response.body)).to eq(
        JSON.parse(
          {
            "error": true,
            "message": "failed to sign in",
            "data": "A confirmation email was sent to your account at 'example1@email.com'. You must follow the instructions in the email before your account can be activated"
          }.to_json
        )
      )
    end
